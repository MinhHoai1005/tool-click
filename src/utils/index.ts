...
export const standardizePhone = (str) => {
...
export const removeCharacterSpecial = (str) => {
    if (!str) {
        return str
    }
    str = str.toString()
...
export const escapseString = (data: string) => {
...
export const sortArray = (array: any, field: string, desc: boolean = true) => {
    if (desc) {
...
export const formatDateLLLL = (data: string | Date): string | Date => {
    if (!data) {
        return ""
    }
    const date = new Date(data)
    ...
    return `${weekday}, ${day} tháng ${month}, ${year} - ${time}`
}

export const print = (id: string) => {
    const mywindow = window.open('', 'PRINT')

    mywindow.document.write('<html><head><meta charset="UTF-8"></head><body>')
    ...
    mywindow.addEventListener('load', async () => {
        mywindow.print()
        mywindow.close()
    })
}
